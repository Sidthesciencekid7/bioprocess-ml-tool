...
<style>
    .main-header {
        background: linear-gradient(135deg, #1a3a4a 0%, #0d6e8a 100%);
        padding: 1.5rem 2rem;
        border-radius: 12px;
    ...
    .flag-warn  { background: #fff7ed; border-left: 4px solid #f97316;
                  padding: 0.5rem 1rem; border-radius: 6px; margin: 0.3rem 0; }
    .flag-ok    { background: #f0fdf4; border-left: 4px solid #22c55e;
                  padding: 0.5rem 1rem; border-radius: 6px; margin: 0.3rem 0; }
</style>
""", unsafe_allow_html=True)

# ── Session State Init ────────────────────────────────────────────────────────
for key in ['df', 'feature_cols', 'target_col', 'time_col', 'group_col',
            'rf', 'xgb', 'y', 'rf_pred', 'xgb_pred', 'flags',
            'df_plot', 'shap_values', 'X_model']:
...
with st.sidebar:
    ...
    nh3_thresh  = st.slider("NH3 max threshold (mM)",       1.0, 20.0, 5.0,  0.5)
    glc_thresh  = st.slider("Glucose min threshold (mM)",   0.1,  5.0, 2.0,  0.1)
    vcd_thresh  = st.slider("Min peak VCD (×10⁶ cells/mL)", 1.0, 50.0, 10.0, 1.0)
    lg_thresh   = st.slider("Lactate/Glucose ratio alert",  0.5,  5.0, 2.0,  0.1)

    st.markdown("---")
    st.markdown("### 🤖 Model Settings")
    n_estimators  = st.slider("Number of trees", 50, 500, 150, 50)
    test_size     = st.slider("Holdout test size (%)", 10, 40, 20, 5)
    use_shap      = st.checkbox("Compute SHAP values (slower)", value=True)

    st.markdown("---")
    st.markdown("### ℹ️ About")
    st.caption("Bioprocess ML Reporter v2.0 — works with CHO, microbial, fermentation, or any time-series bioprocess data.")

# ── Header ────────────────────────────────────────────────────────────────────
st.markdown("""
<div class="main-header">
  <h1>🧬 Bioprocess ML Report Generator</h1>
  <p>Upload any bioprocess dataset → get ML predictions, process flags, SHAP explanations, and downloadable reports.</p>
</div>
""", unsafe_allow_html=True)
...
def smart_detect(cols):
    ...
    numeric_str  = [c for c in cols]
    target = next((c for c in numeric_str for h in target_hints if h.lower() in c.lower()), cols[0])
...
def generate_flags(df, target_col, time_col, nh3_thresh, glc_thresh, vcd_thresh, lg_thresh):
    ...
    if peak > 0 and final < 0.5 * peak:
        flags.append(("warn", f"Final {target_col} ({final:.2f}) is <50% of peak ({peak:.2f}) — possible crash or dilution"))
...
def train_models(X_tuple, y_tuple, groups_tuple, n_estimators):
    X = pd.DataFrame(X_tuple[1], columns=X_tuple[0])
    y = pd.Series(y_tuple[1], name=y_tuple[0])
    groups = pd.Series(groups_tuple) if groups_tuple is not None else None

    rf  = RandomForestRegressor(n_estimators=n_estimators, random_state=42, n_jobs=-1)
    xgb = XGBRegressor(n_estimators=n_estimators, learning_rate=0.05,
    ...
    if groups is not None and groups.nunique() >= 3:
        ...
        rf_pred  = cross_val_predict(rf,  X, y, cv=min(5, len(X)))
        xgb_pred = cross_val_predict(xgb, X, y, cv=min(5, len(X)))

    rf.fit(X, y)
    xgb.fit(X, y)
    return rf, xgb, rf_pred, xgb_pred

def compute_shap(rf, X):
    explainer   = shap.TreeExplainer(rf)
    shap_values = explainer.shap_values(X)
    return shap_values

def make_pdf(df, target_col, time_col, rf, xgb,
             y, rf_pred, xgb_pred, flags,
             titer_path, feat_path, cmp_path, shap_path,
             corr_path, timestamp):
    pdf = FPDF()
    ...
    rows = {
        ...
        "RF R² (CV)":         f"{r2_score(y, rf_pred):.4f}",
        "XGB R² (CV)":        f"{r2_score(y, xgb_pred):.4f}",
...
def make_excel(df, target_col, time_col, rf, xgb, feature_cols,
               y, rf_pred, xgb_pred, flags, timestamp):
    ...
    ws2.append(["R² (CV)", round(r2_score(y, rf_pred), 4), round(r2_score(y, xgb_pred), 4)])
    ws2.append(["MAE",     round(mean_absolute_error(y, rf_pred), 4),
                            round(mean_absolute_error(y, xgb_pred), 4)])
    ws2.append([])
    ...
    for i, row in df_out.iterrows():
        ws4.append([row[time_col], row[target_col],
                    row['RF Predicted'], row['XGB Predicted']])
    chart = LineChart()
...
uploaded = st.file_uploader("Upload CSV or Excel file", type=["csv", "xlsx"],
                             help="Works with CDPpy format Excel files or any tidy CSV.")
...
if st.session_state.df is not None:
    ...
    feature_cols = st.multiselect("Features (inputs to model)",
...
if st.session_state.feature_cols:
    ...
    if st.button("🚀 Run Analysis", type="primary", use_container_width=True):
        ...
        if len(X) == 0:
            st.error("❌ No usable rows after cleaning. Check that your selected feature columns have data.")
            st.stop()
        if len(X) < 6:
            st.error(f"❌ Only {len(X)} usable rows — need at least 6 to train. Try selecting fewer features.")
            st.stop()

        groups = df_plot[group_col].astype(str) if group_col else None
        st.info(f"ℹ️ Training on **{len(X)} rows** with **{len(feature_cols)} features**")

        with st.spinner("Training Random Forest and XGBoost (cross-validated)..."):
            X_tuple = (list(X.columns), X.values.tolist())
            y_tuple = (y.name, y.tolist())
            groups_tuple = groups.tolist() if groups is not None else None
            rf, xgb, rf_pred, xgb_pred = train_models(X_tuple, y_tuple, groups_tuple, n_estimators)

        if use_shap:
            with st.spinner("Computing SHAP values..."):
                shap_values = compute_shap(rf, X)
...
if st.session_state.rf is not None:
    ...
    # ── Plots ──────────────────────────────────────────────────────────────────
    titer_path = shap_path = feat_path = cmp_path = corr_path = None
    ...
    if group_col and group_col in df_plot.columns:
        ...
        selected_batches = st.multiselect(
            "Filter by batch/run", all_batches, default=all_batches, key="batch_filter"
        )
    ...
    if group_col and group_col in df_f.columns:
        ...
        for i, batch in enumerate(df_f[group_col].astype(str).unique()):
            ...
            b_mask_model = df_plot.index.isin(b_idx)
            ax1.plot(df_f.loc[b_idx, time_col], y[b_mask_model], 'o-', color=colors[i%10], label=str(batch))
            ax1.plot(df_f.loc[b_idx, time_col], rf_pred[b_mask_model], 's--', color=colors[i%10], alpha=0.55)
            ax1.plot(df_f.loc[b_idx, time_col], xgb_pred[b_mask_model], '^:', color=colors[i%10], alpha=0.35)
        style_legend = [
    ...
    for ax, pred, name, color in [
        (ax4a, rf_pred,  "Random Forest", "coral"),
        (ax4b, xgb_pred, "XGBoost",       "seagreen")
    ]:
    ...
    fig5, ax5 = plt.subplots(figsize=(min(14, len(corr_cols)*0.9+2), fig5_h))
    mask_tri = np.triu(np.ones_like(corr_df, dtype=bool))
    sns.heatmap(corr_df, mask=mask_tri, annot=True, fmt=".2f",
                cmap="RdYlBu_r", center=0, ax=ax5, linewidths=0.5,
    ...
    with d1:
        try:
            pdf_bytes = make_pdf(
                df_plot, target_col, time_col, rf, xgb,
                y, rf_pred, xgb_pred, flags,
